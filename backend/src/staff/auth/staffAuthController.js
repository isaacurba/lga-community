...
// Helper: sign JWT, set cookie, and return a consistent user payload
// - Auth fields (email/role/verification) come from User
// - Display fields (name) come from Staff profile
const sendAuthResponse = (res, user, staffProfile) => {
  ...
  const userResponse = {
    id: user._id,
    // Staff name is stored on the profile, not on User
  ...
  // This returns the response, so don't call .json() again in the controller
  return res.json({ success: true, token, user: userResponse });
};

export const register = async (req, res) => {
  // FIX: normalize email for consistent lookups
  ...
  try {
    // FIX: enforce global uniqueness via shared User model
    ...
    // FIX: create staff profile linked to shared User record
    const staffProfile = new staffModel({
      name,
      userId: user._id,
    });
...
© ${new Date().getFullYear()} LGA-Connect. All rights reserved.
    ...
    return sendAuthResponse(res, user, staffProfile);
  } catch (error) {
...
export const login = async (req, res) => {
  // FIX: normalize email to avoid case-sensitive lookup misses and duplicates
  ...
  try {
    // FIX: authenticate against shared User model for staff/admin roles only
    // This prevents citizen accounts from logging into staff endpoints.
    ...
    const staffProfile = await staffModel.findOne({ userId: user._id });
    if (!staffProfile) {
      return res.json({ success: false, message: "Staff profile not found" });
    }
...
export const sendVerifyOtp = async (req, res) => {
  try {
    ...
    // FIX: verify OTP is stored on shared User model
    ...
    const displayName = staffProfile?.name || "there";
...
export const sendResetOtp = async (req, res) => {
  ...
  try {
    // FIX: reset OTP stored on shared User model (staff/admin only)
    const user = await User.findOne({
      email,
      role: { $in: ["staff", "admin"] },
    });
...
export const verifyEmail = async (req, res) => {
  ...
  try {
    // FIX: verify against shared User model
    const user = await User.findById(userId);
    if (!user || !["staff", "admin"].includes(user.role)) {
      return res.json({ success: false, message: "User not found" });
    }
...
export const resetPassword = async (req, res) => {
  ...
  try {
    // FIX: password lives in shared User model
    const user = await User.findOne({
      email,
      role: { $in: ["staff", "admin"] },
    }).select("+resetOtpExpireAt");
