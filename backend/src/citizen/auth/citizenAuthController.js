...
const sendAuthResponse = (res, user, citizen) => {
  ...
  const userResponse = {
    ...
    email: user.email,
    ninId: citizen.ninId,
    originalLga: citizen.originalLga,
...
export const registerCitizen = async (req, res) => {
  const { ninId, email, password, firstName, lastName, dob, currentAddress, originalLga, } = req.body;
  ...
  if (
    !ninId || !normalizedEmail || !password || !firstName || !lastName ||!originalLga
  ) {
    return res.json({
  ...
  try {
    ...
    // FIX: enforce global email uniqueness via User model
    ...
    // FIX: create citizen profile linked to shared User record
...
export const citizenLogin = async (req, res) => {
  ...
  try {
    // FIX: authenticate against shared User model and enforce citizen role
    // This keeps staff/admin users from logging in via citizen endpoint.
    const user = await User.findOne({ email, role: "citizen" }).select(
      "+password"
    );
    ...
    const citizen = await citizenModel.findOne({ userId: user._id });
    if (!citizen) {
      return res.json({ success: false, message: "Citizen profile not found" });
    }

    return sendAuthResponse(res, user, citizen);
  } catch (error) {
    return res.json({ success: false, message: error.message });
  }
};
...
export const sendVerifyOtp = async (req, res) => {
  try {
    ...
    // FIX: verification OTP stored on shared User model
    ...
    const displayName = citizenProfile
...
export const citizenResetOtp = async (req, res) => {
  ...
  try {
    // FIX: reset OTP is stored on shared User model
    const user = await User.findOne({ email, role: "citizen" });
    if (!user) {
      return res.json({ success: false, message: "Citizen is not registered" });
    }
    const citizenProfile = await citizenModel.findOne({ userId: user._id });
    const displayName = citizenProfile?.firstName || "there";
    const otp = String(Math.floor(100000 + Math.random() * 900000));
...
export const citizenResetPassword = async (req, res) => {
  ...
  try {
    // FIX: password and reset OTP live in shared User model
    const user = await User.findOne({ email, role: "citizen" }).select(
      "+resetOtpExpireAt"
    );
